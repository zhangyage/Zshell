...
#update:
#2016.03.03 v1.1:
#新增top_size_ip变量，可展示各个访问者IP流量细节
############################################
...
do
	...
	top_size_ip=$(grep $mark "${log_path}"|awk '{if($1 ~ /[0-9]/) print $1}'|sort |awk -F\| '{print $1}'|uniq|while read row;do grep $mark "${log_path}"|grep "$row"|awk -v awk_row=$row '{if($10 ~ /[0-9]/) sum += $10} END {printf("%s" "%0.2f\n",awk_row"\t",sum/1024/1024)}';done|sort -k 2 -rn|head -n ${ip_row:-5})
