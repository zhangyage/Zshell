...
#开始更新
echo `date`
cat /tmp/linshi > /tmp/linshi02
lujing=`cat /tmp/linshi02`
for i in $lujing
    do
        if [ -f $i ]
            then
	...
	#删除上次更新残余文件
	cd /tmp/upload2/ && \rm -rf ./* && cd -
	
	cd $xmbdlj && \rm -rf $a $aa $b $ba $c $d $da $e $f $ff
	echo "rm -rf $a $aa $b $ba $c $d $da $e $f $ff"
	touch $xmbdlj/$a
	...
	unzip -tq $i || sun=3
        ...
        mv $i /tmp/upload2/ && cd /tmp/upload2/
        unzip -q $ysname
	...
	scp $ysname ceshi.lingbug:/home/.tomcat/.zip/upload2/$qhname/
	...
	#删除缓存文件
	ssh root@ceshi.lingbug "cd /home/tomcat/$xmname/work/ && rm -rf ./*"
	...
	#覆盖更新文件
	ssh root@ceshi.lingbug "unzip -d /home/.tomcat/.zip/upload2/$qhname -oq /home/.tomcat/.zip/upload2/$qhname/$ysname"
	ssh root@ceshi.lingbug "echo `date` > /home/.tomcat/.zip/upload2/$qhname/$wjname/update"
	mv $xmbdlj/$ba $xmbdlj/$c
	ssh root@ceshi.lingbug "\cp -r /home/.tomcat/.zip/upload2/$qhname/$wjname/* /home/tomcat/$xmname/webapps/$csxmname-$qhname/"
	ssh root@ceshi.lingbug "/home/tomcat/$xmname/bin/one-startup.sh"
	sleep 5
	ssh root@ceshi.lingbug "\rm -rf /home/.tomcat/.zip/upload2/$qhname/*"
		#判断tomcat是否启动成功
