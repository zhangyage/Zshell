...
#开始更新
echo `date`
cat /tmp/linshi > /tmp/linshi01
lujing=`cat /tmp/linshi01`
for i in $lujing
    do
	if [ -f $i ]
	    then 
	...
	#删除上次更新残余文件
	cd /tmp/upload1/ && \rm -rf ./* && cd -
	echo "\rm -rf /tmp/upload1/*"
	...
	unzip -tq $i || sun=3
        ...
        mv $i /tmp/upload1/ && cd /tmp/upload1/
        unzip -q $ysname
	...
	scp $ysname ceshi.lingbug:/home/.tomcat/.zip/upload1/$qhname/
	...
	#覆盖更新文件
	ssh root@ceshi.lingbug "unzip -d /home/.tomcat/.zip/upload1/$qhname/ -oq /home/.tomcat/.zip/upload1/$qhname/$ysname"
#	ssh root@ceshi.lingbug "echo `date` > /home/.tomcat/.zip/upload1/$qhname/$wjname/update"
	mv $xmbdlj/$ba $xmbdlj/$c
	ssh root@ceshi.lingbug "\cp -r /home/.tomcat/.zip/upload1/$qhname/$wjname/* /home/tomcat/$xmname/webapps/$csxmname-$qhname/"
	ssh root@ceshi.lingbug "/home/tomcat/$xmname/bin/one-startup.sh"
	sleep 5
	ssh root@ceshi.lingbug "\rm -rf /home/.tomcat/.zip/upload1/$qhname/*"
		#判断tomcat是否启动成功
